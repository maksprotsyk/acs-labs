...
void read_all(
        ...
        try {
            if (file.is_regular_file()) {
                ...
                if (std::find(config.archive_file_formats.begin(),
                              ...
                              file_extension)
                    != config.archive_file_formats.end()) {
                    files.push(std::make_pair(read_in_memory(file.path()), true));
                ...
                {
                    files.push(std::make_pair(read_in_memory(file.path()), false));
                }
            }
        }
        catch (const std::exception& e) {
    ...
    for (size_t i = 0; i < config.threads_indexing + 1; i++) {
        files.push(std::make_pair(std::nullptr_t{}, false));
    }
}
